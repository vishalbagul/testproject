    ...
    <div id="main">
      ...
      <div id="left_nav">
        ...
        <ul>
			...
			<li class="border" id="active_link"><a href="#">Other PM</a></li>
        </ul> 
      ...
      <?php
	  	$count_up=0;
		$count_down=0;
		$sql="SELECT * FROM  `pm` WHERE `specification`='other' ORDER BY `pm`.`name`";
		...
		{
			$sql="SELECT * FROM  `pm` WHERE `specification`='other' ORDER BY `pm`.`ip`";
		}
		if($_GET['sort']=="Clustor")
		{
			$sql="SELECT * FROM  `pm` WHERE `specification`='other' ORDER BY `pm`.`specification`";
		}
		if($_GET['sort']=="Status")
		{
			$sql="SELECT * FROM  `pm` WHERE `specification`='other' ORDER BY `pm`.`status`";
		}
