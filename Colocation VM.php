    ...
    <div id="main">
      ...
      <div id="left_nav">
        ...
        <ul>
			...
			<li class="border" id="active_link"><a href="#">Co-Location VM</a></li>
      ...
      <?php
	  	$count_up=0;
		$count_down=0;
		$sql="SELECT * FROM  `vm` WHERE `specification`='Co-Location' ORDER BY `vm`.`name`";
		...
		{
			$sql="SELECT * FROM  `vm` WHERE `specification`='Co-Location' ORDER BY `vm`.`ip`";
		}
		if($_GET['sort']=="Clustor")
		{
			$sql="SELECT * FROM  `vm` WHERE `specification`='Co-Location' ORDER BY `vm`.`specification`";
		}
		if($_GET['sort']=="Status")
		{
			$sql="SELECT * FROM  `vm` WHERE `specification`='Co-Location' ORDER BY `vm`.`status`";
		}
		$res=mysql_query($sql) or die(mysql_error());
	  ?>
      	<div id="show_table">
        <h2>Co-Location Virtual Machines</h2>
