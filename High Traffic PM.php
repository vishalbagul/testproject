    ...
    <div id="main">
      ...
      <div id="left_nav">
        ...
        <ul>
        	...
        	<li class="border" id="active_link"><a href="#">High-Traffic PM</a></li>
      ...
      <?php
	  	$count_up=0;
		$count_down=0;
		$sql="SELECT * FROM  `pm` WHERE `specification`='High Traffic' ORDER BY `pm`.`name`";
		...
		{
			$sql="SELECT * FROM  `pm` WHERE `specification`='High Traffic' ORDER BY `pm`.`ip`";
		}
		if($_GET['sort']=="Clustor")
		{
			$sql="SELECT * FROM  `pm` WHERE `specification`='High Traffic' ORDER BY `pm`.`specification`";
		}
		if($_GET['sort']=="Status")
		{
			$sql="SELECT * FROM  `pm` WHERE `specification`='High Traffic' ORDER BY `pm`.`status`";
		}
		$res=mysql_query($sql) or die(mysql_error());
	  ?>
      	<div id="show_table">
        <h2>High-Traffic Physical Machines</h2>
