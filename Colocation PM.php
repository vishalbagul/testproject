    ...
    <div id="main">
      ...
      <div id="left_nav">
        ...
        <ul>
            ...
            <li class="border" id="active_link"><a href="#">Co-Location PM</a></li>
      ...
      <?php
	  	$count_up=0;
		$count_down=0;
		$sql="SELECT * FROM  `pm` WHERE `specification`='Co-Location' ORDER BY `pm`.`name`";
		...
		{
			$sql="SELECT * FROM  `pm` WHERE `specification`='Co-Location' ORDER BY `pm`.`ip`";
		}
		if($_GET['sort']=="Clustor")
		{
			$sql="SELECT * FROM  `pm` WHERE `specification`='Co-Location' ORDER BY `pm`.`specification`";
		}
		if($_GET['sort']=="Status")
		{
			$sql="SELECT * FROM  `pm` WHERE `specification`='Co-Location' ORDER BY `pm`.`status`";
		}
		$res=mysql_query($sql) or die(mysql_error());
	  ?>
      	<div id="show_table">
        <h2>Co-Location Physical Machines</h2>
