    ...
    <div id="main">
      ...
      <?php
		...
		{	
			...
			if ($status == 0)
			{
				$sql2="UPDATE  `ndc_db`.`pm` SET  `status` =  'UP' WHERE  `pm`.`ip` =  '$ip'";
				$result2=mysql_query($sql2);
			}
			else
			{
				$sql2="UPDATE  `ndc_db`.`pm` SET  `status` =  'DOWN' WHERE  `pm`.`ip` =  '$ip'";
				$result2=mysql_query($sql2);
			}
		}
	  ?>
      	<h2>All Physical Machines</h2>
		<div id="show_table">
      ...
      <?php
		$count_up=0;
		$count_down=0;
		$sql3="SELECT * FROM  `pm` ORDER BY `pm`.`name`";
		...
		{
			$sql3="SELECT * FROM  `pm` ORDER BY `pm`.`ip`";
		}
		if($_GET['sort']=="OperatingSystem")
		{
			$sql3="SELECT * FROM  `pm` ORDER BY `pm`.`os`";
		}
		if($_GET['sort']=="Clustor")
		{
			$sql3="SELECT * FROM  `pm` ORDER BY `pm`.`specification`";
		}
		if($_GET['sort']=="Status")
		{
			$sql3="SELECT * FROM  `pm` ORDER BY `pm`.`status`";
		}
       ...
       <?php 
        ...
        <div style="float:left; width:450px; padding:20px;">
        	<a href="pdf_all_pm.php"><img src="images/save as pdf.png" border="0" /></a>&nbsp;&nbsp;&nbsp;<a href="#"><img src="images/save as cvs.png" border="0" /></a>
        </div>
      </div>
    </div>
</div>
