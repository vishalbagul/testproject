    ...
    <div id="main">
      ...
      <div id="left_nav">
        ...
        <ul>
			...
			<li class="border" id="active_link"><a href="#">Other VM</a></li>
        </ul>
      ...
      <?php
	  	$count_up=0;
		$count_down=0;
		$sql="SELECT * FROM  `vm` WHERE `specification`='other' ORDER BY `vm`.`name`";
		...
		{
			$sql="SELECT * FROM  `vm` WHERE `specification`='other' ORDER BY `vm`.`ip`";
		}
		if($_GET['sort']=="Clustor")
		{
			$sql="SELECT * FROM  `vm` WHERE `specification`='other' ORDER BY `vm`.`specification`";
		}
		if($_GET['sort']=="Status")
		{
			$sql="SELECT * FROM  `vm` WHERE `specification`='other' ORDER BY `vm`.`status`";
		}
